...
class EmissionsRendererModule(BaseModule):
	def __init__(self, emissions, emission_types = [], enable_cell_names=True, save_to_file=False):
		super().__init__()
		...
		for i, emission_type in enumerate(emission_types):
			...
			emissions_matrix = emissions.get_emissions_type_matrix(emission_type)
			im = ax.imshow(emissions_matrix, cmap=cmap, vmin=0, vmax=self.max_values[str(emission_type)],origin='lower')
			...
			# Show cell id
			for cell_id in self.emissions._cells.cells:
				cell = self.emissions._cells.cells[cell_id]
				cell_text = cell_id.replace("poly_","") 
				text = ax.text(cell.matrixPosX, cell.matrixPosY, cell_text, ha="center", va="center", color="w")
	...
	def step(self, timestep):
		...
		for i,emission_type in enumerate(self.emission_types):
			ax = self.axs[i]
			# print(np.max(self.emissions.get_emissions_type_matrix(emission_type)))
