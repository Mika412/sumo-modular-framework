...
class Cell:
    def __init__(self, cell_id, bottom_X, bottom_Y, top_X, top_Y, matrixPosX,
                 matrixPosY):
    ...
    def belongsTo(self, x, y):
        return ((x > self.bottomX) and (x < self.topX) and (y > self.bottomY)
                and (y < self.topY))
...
class CellsModule(BaseModule):
    def __init__(
        ...
        self.cells_to_edges = {}
        self.edge_to_cells = {}
        self.cells_id_matrix = np.empty(shape=(self.yCount, self.xCount),
                                        dtype="S10")
        ...
        for cell_id in self.cells:
            if cell_id in self.cells_to_edges:
                valid_cells_dict[cell_id] = [
                    self.cells[cell_id].matrixPosY,
                    self.cells[cell_id].matrixPosX,
                ]
                valid_cells.append([
                    self.cells[cell_id].matrixPosY,
                    self.cells[cell_id].matrixPosX
    ...
    def load_cells(self):
        ...
        for poly in root.findall("poly"):
            ...
            self.cells[cellId] = Cell(cellId, bottomX, bottomY, topX, topY,
                                      xPos, yPos)
    ...
    def get_neighbors_ids(self, cellId):
        ...
        neighbors = []
        neighbors.append(self.cells_id_matrix[_cell.matrixPosY - 1,
                                              _cell.matrixPosX - 1])
        neighbors.append(self.cells_id_matrix[_cell.matrixPosY - 1,
                                              _cell.matrixPosX])
        neighbors.append(self.cells_id_matrix[_cell.matrixPosY - 1,
                                              _cell.matrixPosX + 1])
        neighbors.append(self.cells_id_matrix[_cell.matrixPosY,
                                              _cell.matrixPosX - 1])
        neighbors.append(self.cells_id_matrix[_cell.matrixPosY,
                                              _cell.matrixPosX + 1])
        neighbors.append(self.cells_id_matrix[_cell.matrixPosY + 1,
    ...
    def close_cell(self, cell_id):
        if (cell_id in self.closed_cells or not cell_id in self.cells
                or not cell_id in self.cells_to_edges):
            return
    ...
    def open_cell(self, cell_id):
        if (not cell_id in self.closed_cells or not cell_id in self.cells
                or not cell_id in self.cells_to_edges):
            return
